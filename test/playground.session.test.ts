...
describe('TrialStore', () => {
  it('hands back the session it was given', () => {
    ...
    const created = store.create(new Session(), 'sanitised', 'gpt-4o-mini');
  ...
  it('issues ids that do not follow from one another', () => {
    const store = new TrialStore();
    const a = store.create(new Session(), 'sanitised', 'm');
    const b = store.create(new Session(), 'sanitised', 'm');
  ...
  it('drops the oldest when it is full', () => {
    ...
    const first = store.create(new Session(), 'sanitised', 'm');
    const second = store.create(new Session(), 'sanitised', 'm');
    const third = store.create(new Session(), 'sanitised', 'm');
  ...
  it('forgets a session once its time is up', () => {
    ...
    const created = store.create(new Session(), 'sanitised', 'm');
  ...
  it('keeps the mapping usable for as long as it holds the session', () => {
    ...
    const created = store.create(session, text, 'm');
